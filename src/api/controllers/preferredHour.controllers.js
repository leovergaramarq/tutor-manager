	...
	if (hours === undefined) {
		...
		db.serialize(() => {
			...
			hours.forEach(({ day, i }) => {
				day.forEach(({ hour, j }) => {
					if (hour === 1) {
						db.all(`SELECT * FROM PreferredHour WHERE Day = ${i} AND Hour = ${j}`, (err, rows) => {
							if (err) {
								console.log(err);
							}
							if (rows?.length) {
								...
								db.run(`INSERT INTO PreferredHour (Day, Hour) VALUES (${i}, ${j})`, err => {
									if (err) {
										console.log(err);
									}
									count++;
						...
						db.run(`DELETE FROM PreferredHour WHERE Day = ${i} AND Hour = ${j}`, err => {
							if (err) {
								console.log(err);
							}
							count++;
